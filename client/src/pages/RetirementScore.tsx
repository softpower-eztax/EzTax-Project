...
const retirementFormSchema = z.object({
  ...
  homeOwnership: z
    .enum(["own_outright", "mortgage", "rent"])
    .default("mortgage"),
  familyStatus: z
...
export default function RetirementScoreStepByStep() {
  ...
  const calculateRetirementScore = (
    ...
    const yearsToRetirement = Math.max(
      1,
      data.expectedRetirementAge - data.currentAge,
    );
    ...
    const investmentGrowth =
      data.currentSavings *
      Math.pow(1 + data.expectedAnnualReturn / 100, yearsToRetirement);
    const contributionGrowth =
      data.expectedAnnualReturn > 0
        ? (annualContribution *
            (Math.pow(1 + data.expectedAnnualReturn / 100, yearsToRetirement) -
              1)) /
    ...
    const totalFutureValue =
      investmentGrowth + contributionGrowth + socialSecurityValue;

    // 인플레이션 조정된 필요 금액
    const inflationAdjustedIncome =
      data.desiredRetirementIncome *
      Math.pow(1 + data.expectedInflationRate / 100, yearsToRetirement);
    ...
    // 부채 비율 점수 (0-5점)
    const debtRatio =
      data.currentIncome > 0 ? data.totalDebt / data.currentIncome : 0;
    ...
    const savingsRate =
      data.currentIncome > 0
        ? (data.monthlyContribution * 12) / data.currentIncome
        : 0;
    if (savingsRate >= 0.2)
      financialHealthScore += 5; // 20% 이상
    else if (savingsRate >= 0.15)
      financialHealthScore += 4; // 15% 이상
    else if (savingsRate >= 0.1)
      financialHealthScore += 3; // 10% 이상
    else if (savingsRate >= 0.05)
      financialHealthScore += 2; // 5% 이상
    else if (savingsRate > 0) financialHealthScore += 1;

    // 투자 수익률 현실성 점수 (0-5점)
    if (data.expectedAnnualReturn >= 5 && data.expectedAnnualReturn <= 8)
      financialHealthScore += 5;
    else if (data.expectedAnnualReturn >= 3 && data.expectedAnnualReturn <= 10)
    ...
    const totalScore = Math.min(
      100,
      Math.max(0, baseScore + financialHealthScore + lifestyleScore),
    );
    ...
    if (preparednessRatio >= 1.2) {
      strengths.push(
        `은퇴 자금이 목표의 ${Math.round(preparednessRatio * 100)}%로 여유있게 준비되어 있습니다`,
      );
      ...
      recommendations.push(
        `월 ${Math.round(monthlyNeeded).toLocaleString()}달러 추가 저축을 권장합니다`,
      );
    } else {
      concerns.push("은퇴 자금이 심각하게 부족합니다");
      recommendations.push(
        `월 ${Math.round(monthlyNeeded).toLocaleString()}달러 추가 저축이 필요하며, 전문가 상담을 권장합니다`,
      );
    }
    ...
    if (emergencyRatio >= 1) {
      ...
      recommendations.push(
        `${Math.round(((6 - emergencyRatio * 6) * data.currentIncome) / 12).toLocaleString()}달러의 추가 비상자금이 필요합니다`,
      );
    }
    ...
    if (savingsRate >= 0.15) {
      strengths.push(
        `우수한 저축률(${Math.round(savingsRate * 100)}%)을 유지하고 있습니다`,
      );
    } else if (savingsRate >= 0.1) {
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-3xl font-bold text-blue-600 mb-2">
                {(
                  (analysis.projectedSavings /
                    (form.getValues("desiredRetirementIncome") * 12 * 25)) *
                  100
                ).toFixed(1)}
                %
              </div>
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              >
                ...
                <ChevronRightIcon
                  className={`h-4 w-4 transition-transform ${showCalculationDetails ? "rotate-90" : ""}`}
                />
              </Button>
            </div>
          </CardHeader>
            ...
            <CardContent className="space-y-6">
              <div className="bg-blue-50 p-4 rounded-lg">
                ...
                <div className="grid md:grid-cols-2 gap-3 text-sm">
                  <div>
                    • 현재 나이: {form.getValues("currentAge")}세 → 은퇴 나이:{" "}
                    {form.getValues("expectedRetirementAge")}세
                  </div>
                  <div>
                    • 은퇴까지 기간:{" "}
                    <strong>
                      {analysis.calculationDetails.yearsToRetirement}년
                    </strong>
                  </div>
                  <div>
                    • 현재 저축: $
                    {form.getValues("currentSavings").toLocaleString()}
                  </div>
                  <div>
                    • 월 저축액: $
                  ...
                  <div>
                    • 목표 월 생활비: $
                    {form.getValues("desiredRetirementIncome").toLocaleString()}
                  </div>
                  <div>
                    • 예상 투자수익률: {form.getValues("expectedAnnualReturn")}%
                  </div>
                  <div>
                    • 연소득: $
                    {form.getValues("currentIncome").toLocaleString()}
                  </div>
                  <div>
                    • Social Security: $
                    {form
                      .getValues("expectedSocialSecurityBenefit")
                      .toLocaleString()}
                    /월
                  </div>
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-green-50 p-4 rounded-lg space-y-2">
                  ...
                  <p className="text-sm text-green-700">
                    ${form.getValues("currentSavings").toLocaleString()} × (1.
                    {String(form.getValues("expectedAnnualReturn")).padStart(
                      2,
                      "0",
                    )}
                    )^{analysis.calculationDetails.yearsToRetirement} =
                    <strong>
                      {" "}
                      $
                      {Math.round(
                        analysis.calculationDetails.investmentGrowth,
                      ).toLocaleString()}
                    </strong>
                  </p>
                </div>
                ...
                <div className="bg-green-50 p-4 rounded-lg space-y-2">
                  ...
                  <p className="text-sm text-green-700">
                    ...
                    <strong>
                      ...
                      $
                      {Math.round(
                        analysis.calculationDetails.contributionGrowth,
                      ).toLocaleString()}
                    </strong>
                  </p>
                </div>
                ...
                <div className="bg-green-50 p-4 rounded-lg space-y-2">
                  ...
                  <p className="text-sm text-green-700">
                    ...
                    <strong>
                      ...
                      $
                      {Math.round(
                        analysis.calculationDetails.socialSecurityValue,
                      ).toLocaleString()}
                    </strong>
                  </p>
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-orange-50 p-4 rounded-lg space-y-2">
                  ...
                  <p className="text-sm text-orange-700">
                    ...
                    {form.getValues("desiredRetirementIncome").toLocaleString()}{" "}
                    × (1.
                    {String(form.getValues("expectedInflationRate")).padStart(
                      2,
                      "0",
                    )}
                    )^{analysis.calculationDetails.yearsToRetirement} =
                    <strong>
                      ...
                      $
                      {Math.round(
                        analysis.calculationDetails.inflationAdjustedIncome,
                      ).toLocaleString()}
                      /월
                    </strong>
                  </p>
                ...
                <div className="bg-orange-50 p-4 rounded-lg space-y-2">
                  ...
                  <p className="text-sm text-orange-700">
                    ...
                    <strong>
                      ...
                      $
                      {Math.round(
                        analysis.calculationDetails.requiredAmount,
                      ).toLocaleString()}
                    </strong>
                  </p>
                </div>
                ...
                <div className="grid md:grid-cols-3 gap-4">
                  <div className="bg-purple-50 p-4 rounded-lg">
                    ...
                    <p className="text-sm text-purple-700 mb-2">
                      준비율:{" "}
                      {Math.round(
                        analysis.calculationDetails.preparednessRatio * 100,
                      )}
                      %
                    </p>
                  ...
                  <div className="bg-purple-50 p-4 rounded-lg">
                    ...
                    <div className="text-xs text-purple-600 space-y-1">
                      <div>
                        • 현재 비상자금:{" "}
                        {Math.round(
                          analysis.calculationDetails.emergencyRatio * 100,
                        )}
                        % (6개월 소득 기준)
                      </div>
                      <div>
                        • 부채비율:{" "}
                        {Math.round(
                          analysis.calculationDetails.debtRatio * 100,
                        )}
                        %
                      </div>
                      <div>
                        • 저축률:{" "}
                        {Math.round(
                          analysis.calculationDetails.savingsRate * 100,
                        )}
                        %
                      </div>
                    </div>
                    <p className="text-lg font-bold text-purple-900 mt-2">
                      {Math.round(
                        analysis.calculationDetails.financialHealthScore,
                      )}
                      점/20점
                    </p>
                  </div>
                  ...
                  <div className="bg-purple-50 p-4 rounded-lg">
                    ...
                    <div className="text-xs text-purple-600 space-y-1">
                      ...
                      <div>
                        • 건강보험:{" "}
                        {form.getValues("hasHealthInsurance") ? "있음" : "없음"}
                      </div>
                      <div>• 주거형태: {form.getValues("homeOwnership")}</div>
                      <div>
                        • 현재 부양가족: {form.getValues("dependentsCount")}명
                        (배우자 제외)
                      </div>
                    </div>
                    <p className="text-lg font-bold text-purple-900 mt-2">
              ...
              <div className="bg-blue-50 border border-blue-200 p-4 rounded-lg">
                ...
                <div className="text-sm text-blue-700 space-y-2">
                  <div>
                    <strong>실직 대비:</strong> 평균 재취업 기간 3-5개월
                    (전문직은 6-12개월)
                  </div>
                  <div>
                    <strong>의료비 응급상황:</strong> 건강보험 본인부담금 및
                    예상치 못한 의료비
                  </div>
                  <div>
                    <strong>주거비 연속성:</strong> 렌트/모기지 등 고정비용 지속
                    지불 능력
                  </div>
                  <div>
                    <strong>심리적 안정:</strong> 급하게 나쁜 조건의 일자리
                    선택하지 않고 냉정한 판단 가능
                  </div>
                  <div className="pt-2 border-t border-blue-300">
                    <strong>개인별 조정:</strong> 안정적 직업(3-4개월),
                    자영업(8-12개월), 맞벌이(6개월 충분)
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-50 border border-gray-200 p-4 rounded-lg">
                ...
                <div className="text-sm text-gray-700 space-y-3">
                  <div>
                    <strong>복리 계산:</strong> William F. Sharpe "Investments"
                    교과서의 미래가치 공식
                    <div className="text-xs text-gray-600 ml-4">
                      FV = PV × (1 + r)^n (현재가치 성장), FV = PMT × [((1 +
                      r)^n - 1) / r] (월 저축 성장)
                    </div>
                  </div>
                  <div>
                    <strong>4% 룰:</strong> Trinity Study (1998) + William
                    Bengen의 Safe Withdrawal Rate 연구
                    <div className="text-xs text-gray-600 ml-4">
                      연간 4% 인출 시 30년간 자금 유지 가능성 95% 이상
                    </div>
                  </div>
                  <div>
                    <strong>Social Security:</strong> Social Security
                    Administration (SSA.gov) 공식 계산법
                    <div className="text-xs text-gray-600 ml-4">
                      조기수령 감액률, 연기수령 증액률 적용
                    </div>
                  </div>
                  <div>
                    <strong>비상자금 6개월:</strong> Federal Reserve
                    가계금융조사 + Dave Ramsey, Suze Orman 권장사항
                    <div className="text-xs text-gray-600 ml-4">
                      평균 실직 기간 통계 및 재정 안정성 연구 기반
                    </div>
                  </div>
                  <div>
                    <strong>저축률/부채비율:</strong> Bureau of Economic
                    Analysis + Consumer Financial Protection Bureau
                    <div className="text-xs text-gray-600 ml-4">
                      미국 가계 재정 통계 및 권장 기준
                    </div>
                  </div>
                  <div>
                    <strong>투자수익률:</strong> S&P 500 Historical Returns +
                    Vanguard 장기 투자 연구
                    <div className="text-xs text-gray-600 ml-4">
                      장기 주식시장 평균 수익률 7-10% 기반
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-yellow-50 border border-yellow-200 p-4 rounded-lg">
                <p className="text-sm text-yellow-800">
                  <strong>💡 이해하기:</strong> 이 점수는 현재 저축 패턴과 재정
                  상태를 기반으로 한 예상 결과입니다. 실제 은퇴 준비는 시장
                  변동성, 건강 상태 변화, 인플레이션 등 다양한 요인의 영향을
                  받을 수 있습니다.
                  <br />
                  <br />
                  <strong>계산 방식:</strong> 학술적 재정학 이론과 실무 재정
                  계획 업계의 베스트 프랙티스를 조합하여 한국 고객에게 맞춘
                  종합적 접근법을 사용합니다.
                </p>
              </div>
            </CardContent>
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-600">{messages.retirementScore.guideDesc}</p>
      </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>
                            {messages.retirementScore.formLabels.currentAge}
                          </FormLabel>
                          <FormControl>
                            <Input
                              type="number"
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>
                            {
                              messages.retirementScore.formLabels
                                .expectedRetirementAge
                            }
                          </FormLabel>
                          <FormControl>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>
                            {messages.retirementScore.formLabels.currentSavings}
                          </FormLabel>
                          <FormControl>
                            <Input
                              type="number"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>
                            {messages.retirementScore.formLabels.currentIncome}
                          </FormLabel>
                          <FormControl>
                            <Input
                              type="number"
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>
                            {messages.retirementScore.formLabels.emergencyFund}
                          </FormLabel>
                          <FormControl>
                            <Input
                              type="number"
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>
                            {messages.retirementScore.formLabels.totalDebt}
                          </FormLabel>
                          <FormControl>
                            <Input
                              type="number"
                        ...
                        <FormItem>
                          ...
                          <div className="flex gap-2">
                            ...
                            <Button
                              ...
                              variant="outline"
                              onClick={() =>
                                setShowSSCalculator(!showSSCalculator)
                              }
                      ...
                      <div className="col-span-full">
                        <Card className="bg-blue-50 border-blue-200">
                          <CardHeader>
                            ...
                            <CardDescription>
                              근무기간과 평균소득을 기반으로 예상 Social
                              Security 연금을 계산합니다
                            </CardDescription>
                            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mt-4">
                              <p className="text-sm text-yellow-800">
                                <strong>⚠️ 안내:</strong> 이 계산기는 대략적인
                                예상 금액을 제공합니다. 정확한 혜택 금액은{" "}
                                <a
                            ...
                            <div className="grid grid-cols-2 gap-4">
                              <div>
                                ...
                                <Input
                                  ...
                                  value={ssStartAge}
                                  onChange={(e) =>
                                    setSsStartAge(Number(e.target.value))
                                  }
                                  min="18"
                                  max="67"
                                />
                              ...
                              <div>
                                ...
                                <Input
                                  ...
                                  value={ssRetireAge}
                                  onChange={(e) =>
                                    setSsRetireAge(Number(e.target.value))
                                  }
                                  min="62"
                                  max="70"
                                />
                              ...
                              <div>
                                ...
                                <Input
                                  ...
                                  value={ssAvgSalary}
                                  onChange={(e) =>
                                    setSsAvgSalary(Number(e.target.value))
                                  }
                                  min="0"
                                  step="1000"
                              ...
                              <div>
                                ...
                                <Input
                                  ...
                                  value={ssClaimAge}
                                  onChange={(e) =>
                                    setSsClaimAge(Number(e.target.value))
                                  }
                                  min="62"
                                  max="70"
                                />
                              ...
                              <div className="text-sm text-gray-600 space-y-1">
                                {(() => {
                                  const workingYears = Math.min(
                                    35,
                                    ssRetireAge - ssStartAge,
                                  );
                                  const annualEarnings = ssAvgSalary; // 이미 연봉
                                  const totalIndexedEarnings =
                                    workingYears * annualEarnings;
                                  const aime = totalIndexedEarnings / (35 * 12);

                                  return (
                                    <>
                                      <div>
                                        • 근무년수: {workingYears}년 / 35년
                                      </div>
                                      <div>
                                        • 연간 소득: $
                                      ...
                                      <div>
                                        • AIME (35년 평균 월소득): $
                                        {Math.round(aime).toLocaleString()}
                                      </div>
                                      <div>
                                        • 수령 조정:{" "}
                                        {ssClaimAge === 67
                                          ? "정상(FRA)"
                                          : ssClaimAge < 67
                                            ? `조기수령(${67 - ssClaimAge}년 빠름)`
                                            : `연기수령(${ssClaimAge - 67}년 늦음)`}
                                      </div>
                                      {workingYears < 10 && (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>
                            {messages.retirementScore.formLabels.healthStatus}
                          </FormLabel>
                          <Select
                          ...
                          >
                            ...
                            <SelectContent>
                              <SelectItem value="excellent">
                                {
                                  messages.retirementScore.options.healthStatus
                                    .excellent
                                }
                              </SelectItem>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>
                            {messages.retirementScore.formLabels.homeOwnership}
                          </FormLabel>
                          <Select
                            onValueChange={field.onChange}
                            defaultValue={field.value}
                          >
                            ...
                            <SelectContent>
                              <SelectItem value="own_outright">
                                {
                                  messages.retirementScore.options.homeOwnership
                                    .ownOutright
                                }
                              </SelectItem>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>
                            {messages.retirementScore.formLabels.familyStatus}
                          </FormLabel>
                          <Select
                            onValueChange={field.onChange}
                            defaultValue={field.value}
                          >
                            ...
                            <SelectContent>
                              <SelectItem value="single">
                                {
                                  messages.retirementScore.options.familyStatus
                                    .single
                                }
                              </SelectItem>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          >
                            ...
                            <SelectContent>
                              <SelectItem value="beginner">
                                {
                                  messages.retirementScore.options
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>
                            {messages.retirementScore.formLabels.riskTolerance}
                          </FormLabel>
                          <Select
                            onValueChange={field.onChange}
                            defaultValue={field.value}
                          >
                            ...
                            <SelectContent>
                              <SelectItem value="conservative">
                                {
                                  messages.retirementScore.options.riskTolerance
                                    .conservative
                                }
                              </SelectItem>
